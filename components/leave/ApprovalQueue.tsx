...
import { Users, CheckCircle2, XCircle, Clock, AlertTriangle, Check, X } from 'lucide-react';
...
const TYPE_COLOR: Record<LeaveType, { bg: string; text: string; border: string }> = {
  Casual:    { bg: 'bg-teal-500/10',   text: 'text-teal-400',   border: 'border-teal-500/20' },
  Sick:      { bg: 'bg-rose-500/10',   text: 'text-rose-400',   border: 'border-rose-500/20' },
  Privilege: { bg: 'bg-violet-500/10', text: 'text-violet-400', border: 'border-violet-500/20' },
  'Comp-off':{ bg: 'bg-amber-500/10',  text: 'text-amber-400',  border: 'border-amber-500/20' },
};
...
function ApprovalCard({ req, userName, userDesig, userAvatar, idx, onApprove, onReject }: CardProps) {
  ...
  return (
    <div
      className={clsx(
        ...
        req.status === 'Rejected' ? 'border-destructive/20 opacity-60' :
        'border-border/30'
      )}
    ...
    >
      ...
      <div className="flex items-center gap-3">
        <div className={clsx('w-10 h-10 rounded-xl flex items-center justify-center font-black text-xs flex-shrink-0', avc)}>
          {userAvatar}
        </div>
        <div className="flex-1 min-w-0">
          <p className="text-sm font-bold text-foreground truncate">{userName}</p>
          <p className="text-[10px] text-foreground-muted truncate">{userDesig}</p>
        </div>
        <span className={clsx('text-[9px] font-bold px-2 py-1 rounded-xl border', tc.bg, tc.text, tc.border)}>
          {req.leaveType}
        </span>
      </div>
...
export default function ApprovalQueue() {
  const { users, activeUserId, leaveRequests, updateLeaveStatus } = useGlobalStore();
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-2">
        {[
          { label: 'Pending',  val: pending,  style: 'bg-amber-500/10 text-amber-400 border-amber-500/20'   },
          { label: 'Approved', val: approved, style: 'bg-success/10 text-success border-success/20'         },
          { label: 'Rejected', val: rejected, style: 'bg-destructive/10 text-destructive border-destructive/20' },
        ].map(({ label, val, style }) => (
          <div key={label} className={clsx('flex-1 flex flex-col items-center py-2 rounded-xl border text-center', style)}>
      ...
      {visibleRequests.length === 0 ? (
        <div className="glass-card rounded-2xl p-8 text-center border border-border/30">
          ...
          <p className="text-sm font-semibold text-foreground-muted">All caught up!</p>
          <p className="text-xs text-foreground-muted/60 mt-1">No leave requests to review right now.</p>
        </div>
      ) : (
