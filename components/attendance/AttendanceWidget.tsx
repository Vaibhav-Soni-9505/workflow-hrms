  ...
  return (
    ...
    >
      ...
      <div>
        ...
        <p className="text-[10px] text-foreground-muted mt-0.5 font-medium">{label}</p>
      </div>
    </div>
  );
}
...
function ClockInModal({ onConfirm, onClose }: { onConfirm: () => void; onClose: () => void }) {
    ...
    <div className="fixed inset-0 z-[100] flex items-end justify-center">
      ...
      <div className="relative w-full max-w-mobile bg-background-secondary rounded-t-3xl border-t border-border/50 animate-slide-up shadow-lg flex flex-col max-h-[85vh] mb-[68px]">
        ...
        <div className="flex-shrink-0 px-5 pt-5">
          ...
          <h2 className="text-base font-black text-foreground mb-1">Clock In</h2>
            ...
            <div className="animate-fade-in flex flex-col items-center gap-3">
              ...
              <div className="relative mx-auto w-36 h-36">
                ...
                {[
                  ...
                  <div
                    key={i}
                    className={clsx('absolute w-4 h-4 rounded-sm border-primary', cls)}
                  />
                ))}
              </div>
            ...
            <div className="animate-fade-in flex flex-col items-center gap-4 w-full">
              ...
              <div className="relative flex items-center justify-center">
                ...
                <div className={clsx(
                  ...
                  {modalStep === 'verified' ? (
                    <CheckCircle2 size={32} className="text-success animate-scale-in" />
                  ) : (
              ...
              <div className="text-center">
                <p className={clsx(
                  ...
                  modalStep === 'verified' ? 'text-success' : 'text-foreground'
                )}>
                  {modalStep === 'verified' ? 'All Checks Passed ✓' : 'Running Verification…'}
                </p>
                <p className="text-[11px] text-foreground-muted mt-0.5">
                  {modalStep === 'verified' ? 'You\'re good to clock in' : 'Please hold on a moment'}
                </p>
              </div>
              ...
              <div className="w-full space-y-2">
                ...
                <div className={clsx(
                  ...
                  <div className={clsx(
                    ...
                    locationOk ? 'bg-success/20 text-success' : 'bg-muted/60 text-foreground-muted'
                  )}>
                    <MapPin size={14} />
                  </div>
                  <div className="flex-1">
                    <p className={clsx('text-xs font-semibold transition-colors duration-300', locationOk ? 'text-success' : 'text-foreground-muted')}>Location</p>
                    <p className="text-[10px] text-foreground-muted/70">{locationOk ? 'Office premises confirmed' : 'Checking GPS…'}</p>
                  </div>
                  {locationOk
                    ? <CheckCircle2 size={16} className="text-success animate-scale-in flex-shrink-0" />
                    : <Loader2 size={14} className="animate-spin text-foreground-muted flex-shrink-0" />
                  }
                </div>
                ...
                <div className={clsx(
                  ...
                  <div className={clsx(
                    'w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0 transition-colors duration-300',
                    ipOk ? 'bg-success/20 text-success' : 'bg-muted/60 text-foreground-muted'
                  )}>
                    <Wifi size={14} />
                  </div>
                  <div className="flex-1">
                    <p className={clsx('text-xs font-semibold transition-colors duration-300', ipOk ? 'text-success' : 'text-foreground-muted')}>Network / IP</p>
                    <p className="text-[10px] text-foreground-muted/70">{ipOk ? 'Corporate network verified' : 'Validating IP address…'}</p>
                  </div>
                  {ipOk
                    ? <CheckCircle2 size={16} className="text-success animate-scale-in flex-shrink-0" />
                    : <Loader2 size={14} className="animate-spin text-foreground-muted flex-shrink-0" />
...
export default function AttendanceWidget({
  ...
  // ── State (local only — checkInTime/clockOutTime live in the page) ──────
  const [clockState, setClockState] = useState<ClockState>(
    checkInTime && !clockOutTime ? 'clocked-in' : 'idle'
  );
  const [showModal, setShowModal]   = useState(false);
  const [elapsedSeconds, setElapsedSeconds] = useState(0);
  const intervalRef = useRef<ReturnType<typeof setInterval> | null>(null);

  // Dynamic summary metrics — updated on clock-out
  ...
  // ── Live elapsed ticker ────────────────────────────────────────────────────
  useEffect(() => {
    if (clockState === 'clocked-in' && checkInTime) {
  ...
  }, [clockState, checkInTime]);

  // ── Handlers ───────────────────────────────────────────────────────────────
  function handleClockIn() {
    const now = new Date();
    onClockIn(now);               // bubble to page
  ...
  function handleClockOut() {
    const now = new Date();
    if (checkInTime) {
      const totalSecs  = Math.floor((now.getTime() - checkInTime.getTime()) / 1000);
      const totalHrs   = totalSecs / 3600;
      const productive = parseFloat((totalHrs * 0.9).toFixed(2));
      const breakHrs   = parseFloat((totalHrs * 0.1).toFixed(2));
      const overtime   = totalHrs > 8 ? parseFloat((totalHrs - 8).toFixed(2)) : 0;
      setSessionMetrics({
  ...
  return (
    <>
        ...
        <div className="relative p-5">
          ...
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <h2 className="text-lg font-black text-white">
                {clockState === 'clocked-in' ? '🟢 You\'re Clocked In' : '👋 Good Morning'}
              </h2>
            </div>
            <div
              className={clsx(
          ...
          {clockState === 'idle' && (
            <div className="text-center py-4 animate-fade-in">
              <p className="text-xs text-white/60 mb-5">Mark your attendance for today</p>
              <button
                id="clock-in-btn"
              ...
              >
                ...
                <div className="w-24 h-24 rounded-full bg-gradient-to-br from-primary to-teal-400 flex flex-col items-center justify-center shadow-teal group-hover:scale-105 group-active:scale-95 transition-transform duration-200">
                  ...
                  <span className="text-[10px] font-bold text-white/90">CLOCK IN</span>
                </div>
              </button>
              <p className="text-[11px] text-white/40 mt-6">Tap to start your shift</p>
            </div>
          )}
          ...
          {clockState === 'clocked-in' && (
            <div className="animate-fade-in">
              ...
              <div className="text-center mb-4">
                <p className="text-[11px] text-white/50 mb-1 font-medium uppercase tracking-wider">Session Time</p>
                ...
                <p className="text-[11px] text-success/80 mt-1">
                  Clocked in at {checkInTime?.toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' })}
                </p>
              </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-2.5">
          <StatCard
            ...
            label="Total Hours"
            value={clockState === 'clocked-in'
              ? formatElapsed(elapsedSeconds).slice(0, 5)
              : formatHours(sessionMetrics.totalHours)}
            color="bg-primary/15"
            delay={0}
          />
          <StatCard
            ...
            value={formatHours(
              clockState === 'clocked-in'
                ? parseFloat((elapsedSeconds / 3600 * 0.9).toFixed(2))
                : sessionMetrics.productiveHours
            )}
            color="bg-accent/15"
            delay={60}
          />
          <StatCard
            ...
            value={formatHours(
              clockState === 'clocked-in'
                ? parseFloat((elapsedSeconds / 3600 * 0.1).toFixed(2))
                : sessionMetrics.breakHours
            )}
          ...
          <StatCard
            ...
            value={formatHours(
              clockState === 'clocked-in'
                ? Math.max(0, parseFloat(((elapsedSeconds / 3600) - 8).toFixed(2)))
                : sessionMetrics.overtimeHours
            )}
      ...
      {/* ── Clock-In Modal ── */}
      {showModal && (
        <ClockInModal onConfirm={handleClockIn} onClose={() => setShowModal(false)} />
      )}
    </>
  );
}
